...
def generate_resource_cpp_content(resource_file_path):
    ...
    resource_data_array = f"{resource_data_name}_resource"
    resource_data_array_size = f"{resource_data_array}_size"
    ...
    cpp_content += f"    static const std::uint8_t {resource_data_array}[] = {{\n"

    hex_lines = [f"{byte}" for byte in binary_data]
    cpp_content += "        " + ", ".join(hex_lines) + "\n"
    cpp_content += f"    }};\n\n"
    
    cpp_content += f"    std::size_t {resource_data_array_size} = sizeof({resource_data_array});\n\n"

    cpp_content += f'    return {{"{resource_file_path}", {resource_data_array}, {resource_data_array_size}, "{MIME_TYPE_MAP.get(file_extension, "application/octet-stream")}"}};\n'
...
def generate_api_cpp_content(pack_name, mount_methods):
    cpp_content = f'#include "deskgui/resource_compiler.h"\n\n'
    cpp_content += f"using namespace deskgui;\n\n"
...
def extend_api_cpp_content(existing_content, pack_name, mount_methods):
...
def generate_api_cpp(pack_name, resource_compiler_cpp, mount_methods):
    if os.path.exists(resource_compiler_cpp):
        with open(resource_compiler_cpp, "r") as cpp_file:
            existing_content = cpp_file.read()

        cpp_content = extend_api_cpp_content(existing_content, pack_name, mount_methods)
    else:
        cpp_content = generate_api_cpp_content(pack_name, mount_methods)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate C++ files for packed resource"
    )
    parser.add_argument("output_dir", help="Output directory for generated files")
    parser.add_argument("pack_name", help="Name for the packed resource library")
    parser.add_argument(
        "resource_compiler_cpp", help="Name for the packed resource library"
    )
    parser.add_argument(
        "resource_files", nargs="+", help="List of resource files to be packed"
    )
    ...
    generate_api_cpp(args.pack_name, args.resource_compiler_cpp, mount_methods)
