...
: pImpl_(std::make_unique<Impl>()), appHandler_(appHandler), name_(name) {
    ...
    pImpl_->navigationDelegate = [[CustomNavigationDelegate alloc] initWithWebview:this resources:&resources_];
    [pImpl_->controller addScriptMessageHandler:pImpl_->navigationDelegate name:kScriptMessageCallback];
